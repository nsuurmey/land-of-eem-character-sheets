...
You are building a Discord bot for a play-by-post *Land of Eem* game. The full, authoritative plan is in `BUILD_SPEC.md`. Read it first and implement its sections **in order**. Follow it literally; do not redesign the schema, command surface, or roll logic.
...
- The d12 result bands must match `BUILD_SPEC.md` §5 exactly: 1–2 Complete Failure, 3–5 Failure with a Plus, 6–8 Success with a Twist, 9–11 Success, 12+ Complete Success. Bands read off the die+modifier total (see the CONFIRM note).
...
## Done means
All boxes in `BUILD_SPEC.md` §11 pass.
